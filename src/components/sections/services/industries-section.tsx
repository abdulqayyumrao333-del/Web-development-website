import { HeartPulse, GraduationCap, Layers, ShoppingCart, Truck, Megaphone, Landmark, Home, Factory, Sparkles, ArrowRight } from "lucide-react";
...
export async function IndustriesSection() {
  let industries: Awaited<ReturnType<typeof db.industry.findMany>> = [];
  try {
    industries = await db.industry.findMany({ where: { visible: true }, orderBy: { order: "asc" } });
  } catch {
    industries = [];
  }
          ...
          <div className="lg:sticky lg:top-28">
            ...
            <div className="relative mb-4">
              <h2 className="relative text-h2 font-semibold tracking-tight leading-tight">
                Where this
                <br />
                <span className="text-accent-indigo">expertise applies</span>
              </h2>
            </div>

            <p className="text-base text-text-secondary leading-relaxed mt-3 max-w-[14rem]">
              Industries and sectors where I've built solutions and delivered value.
            </p>
            ...
            <div className="mt-8 hidden lg:flex flex-col gap-1.5">
              {industries.map((industry, i) => {
                const Icon = ICONS[industry.name] ?? Layers;
                return (
                  <div key={industry.id} className="flex items-center gap-2 group cursor-default">
          ...
          <div className="relative">
            ...
            <div className="grid gap-3.5 sm:grid-cols-2 lg:grid-cols-3">
              {industries.map((industry, i) => {
                const Icon = ICONS[industry.name] ?? Layers;
                return (
                  <Reveal key={industry.id} delay={0.12 + i * 0.05}>
                    <div
            ...
            <div className="mt-3.5 flex items-center justify-between rounded-lg border border-accent-indigo/10 bg-accent-indigo/[0.025] px-4 py-2.5">
              <p className="font-mono text-[11px] text-text-muted">
                INDUSTRIES · {industries.length} SECTORS · DIVERSE EXPERTISE
              </p>
