...
import { Sparkles, Layers, Code2, Server, Zap } from "lucide-react";
...
export async function ServicesGrid() {
  ...
  try {
    [services, projects] = await Promise.all([
      db.service.findMany({ where: { visible: true }, orderBy: { order: "asc" } }),
  ...
  // Count related projects per service
  const serviceProjectCounts = services.map((service) => ({
    id: service.id,
    count: projects.filter((p) => p.categories.includes(service.category)).length,
  }));
        ...
        <Reveal>
          <div className="lg:sticky lg:top-28">
            ...
            <p className="text-base text-text-secondary leading-relaxed mt-3 max-w-[14rem]">
              Select any service below to see the full breakdown — deliverables, tech stack, and related work.
            </p>
            ...
            <div className="mt-8 hidden lg:flex flex-col gap-1.5">
              {services.map((service, i) => {
                const count = serviceProjectCounts.find((s) => s.id === service.id)?.count || 0;
                return (
                  <div key={service.id} className="flex items-center gap-2 group cursor-default">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="mt-0.5 flex items-center justify-between rounded-lg border border-accent-indigo/10 bg-accent-indigo/[0.025] px-4 py-2.5">
                  <p className="font-mono text-[11px] text-text-muted">
                    SERVICES · {services.length} OFFERINGS · {projects.length} PROJECTS
                  </p>
