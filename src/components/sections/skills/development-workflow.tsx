import { Lightbulb, ClipboardList, Palette, Code2, TestTube2, Rocket, Wrench, ArrowRight, CheckCircle } from "lucide-react";
import { Reveal } from "@/components/sections/reveal";

const STEPS = [
  { icon: Lightbulb, title: "Idea", phase: "Start" },
  { icon: ClipboardList, title: "Planning", phase: "Start" },
  { icon: Palette, title: "Design", phase: "Design" },
  { icon: Code2, title: "Development", phase: "Build" },
  { icon: TestTube2, title: "Testing", phase: "Build" },
  { icon: Rocket, title: "Deployment", phase: "Ship" },
  { icon: Wrench, title: "Maintenance", phase: "Ship" },
];
...
const phaseColors: Record<string, string> = {
  "Start": "from-blue-500/20 to-cyan-500/20",
          ...
          <div className="lg:sticky lg:top-28">
            ...
            <div className="relative mb-4">
              <h2 className="relative text-h2 font-semibold tracking-tight leading-tight">
                From idea
                <br />
                <span className="text-accent-indigo">to shipped</span>
              </h2>
            </div>

            <p className="text-base text-text-secondary leading-relaxed mt-3 max-w-[14rem]">
              My development workflow — a proven process from concept to production.
            </p>
            ...
            <div className="mt-8 hidden lg:flex flex-col gap-2">
              {[...new Set(STEPS.map(s => s.phase))].map((phase, i) => {
                const phaseSteps = STEPS.filter(s => s.phase === phase);
                const Icon = phase === "Start" ? Lightbulb : 
                           phase === "Design" ? Palette :
                           phase === "Build" ? Code2 : Rocket;
                return (
        ...
        <Reveal delay={0.1}>
          <div className="relative">
            ...
            <div className="flex flex-col gap-3">
              {STEPS.map(({ icon: Icon, title, phase }, i) => {
                      ...
                      <div className="relative flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex gap-1 mt-1.5">
                            {phaseSteps.map((_, idx) => (
                              <span
                                key={idx}
                                className="h-1 rounded-full transition-all duration-300"
                                style={{
                                  width: idx + 1 <= stepInPhase ? "0.75rem" : "0.25rem",
                                  backgroundColor: idx + 1 <= stepInPhase
                                    ? `rgb(99 102 241 / ${0.6 - idx * 0.1})`
                                    : "rgb(99 102 241 / 0.08)",
                                }}
                              />
                            ))}
            ...
            <div className="mt-3.5 flex items-center justify-between rounded-lg border border-accent-indigo/10 bg-accent-indigo/[0.025] px-4 py-2.5">
              <p className="font-mono text-[11px] text-text-muted">
                WORKFLOW · {STEPS.length} STAGES · {[...new Set(STEPS.map(s => s.phase))].length} PHASES
              </p>
