...
export async function RelatedProjects({ 
  ...
  let related: Awaited<ReturnType<typeof db.project.findMany>> = [];
  try {
    related = await db.project.findMany({
      where: { 
          ...
          <div className="lg:sticky lg:top-28">
            ...
            <div className="relative mb-4">
              <h2 className="relative text-h2 font-semibold tracking-tight leading-tight">
                More
                <br />
                <span className="text-accent-indigo">like this</span>
              </h2>
            </div>
            ...
            <div className="mt-8 hidden lg:flex flex-wrap gap-1.5">
              {categories.map((cat) => (
                <span
                  key={cat}
                  className="inline-flex items-center gap-1 rounded-full border border-accent-indigo/12 bg-accent-indigo/[0.03] px-2.5 py-1 text-[9px] font-mono uppercase tracking-wider text-accent-indigo/50"
                >
                  <span className="h-1 w-1 rounded-full bg-accent-indigo/30" />
                  {cat}
                </span>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="mt-0.5 flex items-center justify-between rounded-lg border border-accent-indigo/10 bg-accent-indigo/[0.025] px-4 py-2.5">
                <p className="font-mono text-[11px] text-text-muted">
                  RELATED · {related.length} PROJECTS · SAME CATEGORY
                </p>
