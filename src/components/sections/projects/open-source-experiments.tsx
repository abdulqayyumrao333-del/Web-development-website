...
import { FlaskConical, Github, Star, GitFork, ExternalLink, Sparkles } from "lucide-react";
...
interface OpenSourceExperimentsProps {
  excludeUrls: string[];
  title?: string;
  description?: string;
}

export function OpenSourceExperiments({ 
  excludeUrls, 
  title = "Open Source Experiments",
  description = "Side projects, experiments, and open source contributions beyond featured work."
}: OpenSourceExperimentsProps) {
        ...
        <div className="grid lg:grid-cols-[minmax(0,15rem)_1fr] gap-10 lg:gap-16 items-start">
          <div className="lg:sticky lg:top-28">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div className="h-4 w-24 bg-accent-indigo/10 rounded animate-pulse" />
            </div>
            <div className="h-8 w-48 bg-accent-indigo/10 rounded animate-pulse mb-3" />
            <div className="h-4 w-32 bg-accent-indigo/5 rounded animate-pulse" />
          </div>
          <div className="flex flex-wrap gap-2">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="h-10 w-24 bg-accent-indigo/5 rounded-full animate-pulse" />
            ))}
          </div>
        </div>
      </div>
  ...
  const experiments = repos?.filter((r) => !excludeUrls.includes(r.url) && !r.isPinned) ?? [];
    ...
    return (
      <section className="relative mx-auto max-w-6xl px-6 py-20 sm:py-28">
        ...
        <div className="grid lg:grid-cols-[minmax(0,15rem)_1fr] gap-10 lg:gap-16 items-start">
          ...
          <div className="lg:sticky lg:top-28">
            ...
            <div className="relative mb-4">
              <h2 className="relative text-h2 font-semibold tracking-tight leading-tight">
                Open Source
                <br />
                <span className="text-accent-indigo">Experiments</span>
              </h2>
            </div>
          ...
          >
            <FlaskConical className="h-10 w-10 text-accent-indigo/30 mx-auto mb-4" strokeWidth={1.5} />
            <p className="text-sm text-text-secondary max-w-sm mx-auto">
              Nothing here yet beyond the featured and pinned work above.
            </p>
          </div>
        </div>
      </section>
        ...
        <Reveal>
          <div className="lg:sticky lg:top-28">
            ...
            <div className="mt-6 flex items-stretch gap-3">
              ...
              <div className="space-y-2.5 text-xs text-text-muted font-mono">
                <p className="flex items-center gap-1.5">
                  ...
                  {experiments.length} repositories
                </p>
                <p className="flex items-center gap-1.5">
            ...
            <div className="mt-8 hidden lg:flex flex-col gap-1.5">
              {experiments.slice(0, 6).map((repo) => (
                <div key={repo.name} className="flex items-center gap-2 group cursor-default">
                  <span className="font-mono text-[10px] text-accent-indigo/25 w-4 text-right group-hover:text-accent-indigo/50 transition-colors">
                    {String(experiments.indexOf(repo) + 1).padStart(2, "0")}
                  </span>
              ...
              {experiments.length > 6 && (
                <div className="flex items-center gap-2 text-[10px] text-text-muted/30">
                  <span className="w-4 text-right">+{experiments.length - 6}</span>
                  <span className="h-px w-3 bg-accent-indigo/8" />
          ...
          <div className="relative">
            ...
            <div className="flex flex-col gap-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {experiments.map((repo, i) => (
                  <Reveal key={repo.name} delay={0.12 + i * 0.04}>
                    <a
                      href={repo.url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="group relative overflow-hidden rounded-xl border border-accent-indigo/12 bg-gradient-to-br from-bg-surface-1/95 to-bg-surface-1/70 backdrop-blur-sm transition-all duration-400 hover:border-accent-indigo/30 hover:shadow-lg hover:shadow-accent-indigo/5 hover:-translate-y-0.5 p-4 block"
                      style={{ boxShadow: panelShadow }}
                    >
                      {/* hover gradient */}
                      <div
                      ...
                      <div className="relative">
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-center gap-2 min-w-0">
                            <div className="flex h-8 w-8 items-center justify-center rounded-lg border border-accent-indigo/12 bg-accent-indigo/6 group-hover:bg-accent-indigo/12 transition-all duration-300 group-hover:scale-105 shrink-0">
                          ...
                          <ExternalLink className="h-3.5 w-3.5 text-text-muted/20 group-hover:text-accent-indigo/40 transition-colors shrink-0 mt-0.5" strokeWidth={1.5} />
                        </div>
                        ...
                        <div className="mt-2.5 flex items-center gap-3 text-[10px] text-text-muted/50">
                          {repo.language && (
              ...
              <div className="mt-0.5 flex items-center justify-between rounded-lg border border-accent-indigo/10 bg-accent-indigo/[0.025] px-4 py-2.5">
                <p className="font-mono text-[11px] text-text-muted">
                  OPEN SOURCE · {experiments.length} REPOS · EXPERIMENTS
                </p>
                <div className="flex gap-1">
                  {experiments.slice(0, 6).map((_, i) => (
                    <span
                      key={i}
