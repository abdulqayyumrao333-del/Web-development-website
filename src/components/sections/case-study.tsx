...
import { Search, ClipboardList, Code2, AlertTriangle, Lightbulb, TrendingUp, BookOpen, Sparkles } from "lucide-react";

const SECTIONS: { key: keyof Project; label: string; icon: React.ReactNode }[] = [
  { key: "caseStudyResearch", label: "Research", icon: <Search className="h-4 w-4" strokeWidth={1.75} /> },
  { key: "caseStudyPlanning", label: "Planning", icon: <ClipboardList className="h-4 w-4" strokeWidth={1.75} /> },
  { key: "description", label: "Development", icon: <Code2 className="h-4 w-4" strokeWidth={1.75} /> },
  { key: "caseStudyChallenges", label: "Challenges", icon: <AlertTriangle className="h-4 w-4" strokeWidth={1.75} /> },
  { key: "caseStudySolution", label: "Solution", icon: <Lightbulb className="h-4 w-4" strokeWidth={1.75} /> },
  { key: "caseStudyResults", label: "Results", icon: <TrendingUp className="h-4 w-4" strokeWidth={1.75} /> },
  { key: "caseStudyLessons", label: "Lessons Learned", icon: <BookOpen className="h-4 w-4" strokeWidth={1.75} /> },
];
          ...
          <div className="lg:sticky lg:top-28">
            ...
            <div className="relative mb-4">
              <h2 className="relative text-h2 font-semibold tracking-tight leading-tight">
                Project
                <br />
                <span className="text-accent-indigo">journey</span>
              </h2>
            </div>

            <p className="text-base text-text-secondary leading-relaxed mt-3 max-w-[14rem]">
              A deep dive into the process, challenges, and outcomes of this project.
            </p>
            ...
            <div className="mt-8 hidden lg:flex flex-col gap-1.5">
              {sections.map((s, i) => (
                <div key={s.key as string} className="flex items-center gap-2 group cursor-default">
                  ...
                  <span className="text-accent-indigo/20 group-hover:text-accent-indigo/40 transition-colors">
                    {s.icon}
                  </span>
        ...
        <Reveal delay={0.1}>
          <div className="relative">
            ...
            <div className="flex flex-col gap-4">
              {sections.map((s, i) => {
                const content = project[s.key] as string;
                return (
                  <Reveal key={s.key as string} delay={0.12 + i * 0.06}>
                    <div
                      ...
                      <div className="relative">
                        <div className="flex items-center gap-3 mb-3">
                          <div className="flex h-9 w-9 items-center justify-center rounded-lg border border-accent-indigo/12 bg-accent-indigo/6 group-hover:bg-accent-indigo/12 transition-all duration-300 group-hover:scale-105 shrink-0">
                            <span className="text-accent-indigo">{s.icon}</span>
                          </div>
                          <div>
                            <span className="text-[9px] font-mono text-accent-indigo/30 group-hover:text-accent-indigo/50 transition-colors">
                              {String(i + 1).padStart(2, "0")}
                            </span>
                          ...
                          <span className="ml-auto text-[8px] font-mono text-accent-indigo/8 group-hover:text-accent-indigo/20 transition-colors">
                            {s.key as string}
                          </span>
                        </div>
            ...
            <div className="mt-2 flex items-center justify-between rounded-lg border border-accent-indigo/10 bg-accent-indigo/[0.025] px-4 py-2.5">
              <p className="font-mono text-[11px] text-text-muted">
                CASE STUDY · {sections.length} SECTIONS · DEEP DIVE
              </p>
